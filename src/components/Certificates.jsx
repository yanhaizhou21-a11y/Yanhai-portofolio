      ...
      <div className="container">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {(certificates || []).map((item) => (
            <button key={item.id || item.name} onClick={() => setActive(item)} className="rounded-xl border p-3 text-left" style={{ borderColor: 'var(--border)', background: 'var(--card-bg)' }}>
              <img src={item.image || 'https://placehold.co/800x600'} alt={item.name} className="h-40 w-full rounded-lg object-cover" loading="lazy" />
      ...
      {active && (
        <div className="fixed inset-0 z-[100] grid place-items-center bg-black/60 p-4 backdrop-blur-sm" onClick={() => setActive(null)}>
          <article className="w-full max-w-3xl rounded-xl border p-4" style={{ borderColor: 'var(--border)', background: 'var(--bg)' }} onClick={(e) => e.stopPropagation()}>
            <img src={active.image || 'https://placehold.co/1400x900'} alt={active.name} className="h-[28rem] w-full rounded-lg object-contain" />
